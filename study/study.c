...
Point study_point_new_lit(int32_t const x, int32_t const y) { return CERIVE_NEW(Point, .x = x, .y = y); }
...
enum cerive_ordering study_point_cmp(Point const *const a, Point const *const b) { return Point_cmp(a, b); }
...
Frame study_frame_new_lit(Line const e, int32_t const id) { return CERIVE_NEW(Frame, .edge = e, .id = id); }
...
enum cerive_ordering study_frame_cmp(Frame const *const a, Frame const *const b) { return Frame_cmp(a, b); }
...
enum cerive_ordering study_span_cmp(Span const *const a, Span const *const b) { return Span_cmp(a, b); }
...
enum cerive_ordering study_boxed_cmp(Boxed const *const a, Boxed const *const b) { return Boxed_cmp(a, b); }
