...
int study_point_debug(Point const * const s, char * const b, size_t const n) { return Point_debug(
	s,
	b,
	n
); }
Point study_point_new(int32_t const x, int32_t const y) { return Point_new(x, y); }
Point study_point_new_lit(int32_t const x, int32_t const y) { return CERIVE_NEW(
	Point,
	.x = x,
	.y = y
); }
...
enum cerive_ordering study_point_cmp(Point const * const a, Point const * const b) { return Point_cmp(
	a,
	b
); }
size_t study_point_hash(Point const * const s) { return Point_hash(s); }

int study_frame_debug(Frame const * const s, char * const b, size_t const n) { return Frame_debug(
	s,
	b,
	n
); }
Frame study_frame_new(Line const e, int32_t const id) { return Frame_new(e, id); }
Frame study_frame_new_lit(Line const e, int32_t const id) { return CERIVE_NEW(
	Frame,
	.edge = e,
	.id = id
); }
...
enum cerive_ordering study_frame_cmp(Frame const * const a, Frame const * const b) { return Frame_cmp(
	a,
	b
); }
size_t study_frame_hash(Frame const * const s) { return Frame_hash(s); }

int study_span_debug(Span const * const s, char * const b, size_t const n) { return Span_debug(
	s,
	b,
	n
); }
Span study_span_new(Point *const first, Point **const rows, int32_t const len) { return Span_new(
	first,
	rows,
	len
); }
bool study_span_eq(Span const * const a, Span const * const b) { return Span_eq(a, b); }
enum cerive_ordering study_span_cmp(Span const * const a, Span const * const b) { return Span_cmp(
	a,
	b
); }
size_t study_span_hash(Span const * const s) { return Span_hash(s); }

int study_boxed_debug(Boxed const * const s, char * const b, size_t const n) { return Boxed_debug(
	s,
	b,
	n
); }
...
enum cerive_ordering study_boxed_cmp(Boxed const * const a, Boxed const * const b) { return Boxed_cmp(
	a,
	b
); }
size_t study_boxed_hash(Boxed const * const s) { return Boxed_hash(s); }

int study_shape_debug(Shape const * const s, char * const b, size_t const n) { return Shape_debug(
	s,
	b,
	n
); }
