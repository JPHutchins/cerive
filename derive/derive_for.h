 ...
 * compose by value and Debug / PartialEq recurse into the inner type. Debug
 * writes into the caller's buffer at a running offset and returns the snprintf
 * would-be length, so the caller sizes with the two-pass idiom:
 *     int const need = T_debug(&v, NULL, 0); char buf[need + 1]; ...
 *
 * Feature-equal to derive_hybrid.h. FOR_EACH_C yields a comma-separated, no-
 * trailing-comma parameter list (so no DROP1 needed), but the EXPAND nesting
 * caps the field count at the unroll depth (~27 here).
 */
